...
public:
  ...
  ~ShellSort() { delete[] num_segment_list; }

  void sortSegment(T array[], int size, int segment_idx, int cur_segment_total,
                   int (*comparator)(T &, T &))
  {
    int current;
    int walker;
    ...
    while (current < size)
    {
      ...
      {
        array[walker + cur_segment_total] = array[walker]; // shift to right
