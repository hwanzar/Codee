...
public:
    ...
    string toString(string (*entry2str)(K &, V &) = 0, bool avl = false)
    {
        if (this->pRoot == 0)
            ...
            return this->pRoot->toString(entry2str, avl);
    }
...
{
    if (pRoot == 0)
        return new BST<K, V>::Node(data, 0, 0);
    ...
    remove(Node *pRoot, K key, bool &success, V &retValue)
{
    ...
    else
    {
        ...
        else
        {
            ...
            while (pLargestOnLeft->pRight != 0)
                pLargestOnLeft = pLargestOnLeft->pRight;
